...
class ResponseTest extends TestCase
{
    ...
    public function testWithExpires(): void
    {
        ...
        $this->assertSame($now->format('D, d M Y H:i:s \G\M\T'), $new->getHeaderLine('Expires'));

        $now = time();
        $new = $response->withExpires($now);
        $this->assertSame(gmdate('D, d M Y H:i:s \G\M\T'), $new->getHeaderLine('Expires'));

        $time = new NativeDateTime('+1 day', new DateTimeZone('UTC'));
        $new = $response->withExpires('+1 day');
        $this->assertSame($time->format('D, d M Y H:i:s \G\M\T'), $new->getHeaderLine('Expires'));
    }
