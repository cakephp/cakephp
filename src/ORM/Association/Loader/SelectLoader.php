...
class SelectLoader
{
    ...
    protected function rewriteJoinIdentifier(SelectQuery $query, mixed $field, string $aliasedTable): mixed
    {
        ...
        if (
            ...
            $driver->supports(DriverFeature::CASE_SENSITIVE_QUOTED_IDENTIFIERS)
        ) {
    ...
    protected function buildSubquery(SelectQuery $query): SelectQuery
    {
        ...
        $useDistinct = !$filterQuery->clause('group') && $driver->supports(DriverFeature::SUBQUERY_FILTER_DISTINCT);
        $fields = $this->subqueryFields($query, $useDistinct);
     ...
     */
    protected function subqueryFields(SelectQuery $query, bool $includeOrderFields = false): array
    {
