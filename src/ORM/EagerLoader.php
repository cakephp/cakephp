...
class EagerLoader
{
    ...
    /**
     * Resolved associations with renamed aliases to handle duplicates.
     *
     * @var array<string, \Cake\ORM\EagerLoadable>|null
     */
    protected ?array $_resolvedJoins = null;
    ...
    public function attachableAssociations(Table $repository): array
    {
        ...
        $this->_resolvedJoins = $this->_resolveJoins($contain, $matching);

        return $this->_resolvedJoins;
    }
     ...
     *
     * NOTE: This method no longer changes strategies for nested associations.
     * The _mergeNestedJoins() method handles alias disambiguation by renaming
     * conflicting associations (e.g., Creator.Comments -> Creator_Comments).
     * This allows nested associations to be properly joined with unique aliases.
     *
     * @return void
     */
    protected function _fixStrategies(): void
    {
        // No longer needed - _mergeNestedJoins() handles alias conflicts
    }
    ...
    public function associationsMap(Table $table): array
    {
        ...
        // Use resolved joins if available (which includes renamed aliases for duplicates),
        // otherwise fall back to normalized containments
        $containMap = $this->_resolvedJoins ?? $this->normalized($table);
        $map = $this->_buildAssociationsMap($map, $containMap);
