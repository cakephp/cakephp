...
class Hash
{
    ...
    public static function get(ArrayAccess|array $data, array|string|int|null $path, mixed $default = null): mixed
    {
        ...
        $parts = is_string($path) || is_int($path) ? explode('.', (string)$path) : $path;
    ...
    public static function extract(ArrayAccess|array $data, string $path): ArrayAccess|array
    {
        ...
        $tokens = str_contains($path, '[') ? Text::tokenize($path, '.', '[', ']') : explode('.', $path);
    ...
    protected static function _matches(ArrayAccess|array $data, string $selector): bool
    {
        ...
        foreach ($conditions as $cond) {
            ...
            $attrPresent = is_array($data) ? array_key_exists($attr, $data) : $data->offsetExists($attr);
    ...
    public static function nest(array $data, array $options = []): array
    {
        ...
        $options += [
            'idPath' => sprintf('{n}.%s.id', $alias),
            'parentPath' => sprintf('{n}.%s.parent_id', $alias),
            'children' => 'children',
            'root' => null,
        ];
        $return = [];
        ...
        $root = $options['root'] ?: static::get($return[0], $parentKeys);
