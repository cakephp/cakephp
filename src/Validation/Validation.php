...
class Validation
{
     ...
     * ISBN 10 Validation
     *
     * @link http://stackoverflow.com/questions/14095778/regex-differentiating-between-isbn-10-and-isbn-13
     * @param string $isbn ISBN
     * @return bool
     */
    public static function containsISBN($isbn)
    {
    ...
    /**
     * Strips any special chars from a ISBN string.
     *
     * @param string $isbn ISBN.
     * @return string
     */
    protected static function _stripISBNSpecialChars($isbn)
    {
        return str_replace([':', '-', 'ISBN'], '', $isbn);
    }
     ...
     * @param string|array|null $type Type of ISBN, isbn10 or isb13, use the constants.
     * @return bool
     */
    public static function isbn($isbn, $type = null)
    {
        if (is_array($type)) {
            $type = null;
        }

        if (static::containsISBN($isbn) === false) {
            return false;
        }

        $isbn = static::_stripISBNSpecialChars($isbn);

        if (empty($type)) {
            return static::_isbn10($isbn) || static::_isbn13($isbn);
        }
        if ($type === static::ISBN_13) {
            return static::_isbn13($isbn);
        }
        if ($type === static::ISBN_10) {
            return static::_isbn10($isbn);
        }
     ...
     * @return bool
     */
    protected static function _isbn10($isbn)
    {
        $check = 0;
     ...
     * @return bool
     */
    protected static function _isbn13($isbn)
    {
        $check = 0;
