    ...
    protected function _list($name = null)
    {
        ...
        foreach ($pluginsList as $plugin) {
            ...
            if (!is_dir($path)) {
                ...
                $this->out(
                        sprintf('Skipping plugin %s. It does not have webroot folder.', $plugin), 2, Shell::VERBOSE
                );
                continue;
            }
    ...
    protected function _process($plugins, $copy = false)
    {
        foreach ($plugins as $plugin => $config) {
            ...
            if (file_exists($config['destDir'] . $config['link'])) {
                $this->out(
                        $config['destDir'] . $config['link'] . ' already exists', 1, Shell::VERBOSE
                );
                continue;
            }
            ...
            if (!$copy) {
                $result = $this->_createSymlink(
                        $config['srcPath'], $config['destDir'] . $config['link']
                );
                if ($result) {
                    continue;
                }
