...
class CsrfProtectionMiddleware implements MiddlewareInterface
{
    ...
    public function process(ServerRequestInterface $request, RequestHandlerInterface $handler): ResponseInterface
    {
        ...
        $cookieData = $this->_getCookieToken($request);
    ...
    {
        $cookie = $this->_getCookieToken($request);
     ...
     *
     * @param \Psr\Http\Message\ServerRequestInterface $request The request to get cookie from.
     * @return string|null Valid existing token or null
     */
    protected function _getCookieToken(ServerRequestInterface $request): ?string
    {
        $cookies = $request->getCookieParams();
        $cookieData = Hash::get($cookies, $this->_config['cookieName']);

        if ($cookieData === null || strlen($cookieData) <= self::TOKEN_VALUE_LENGTH) {
            return null;
        }

        $cookieKey = (string)substr($cookieData, 0, static::TOKEN_VALUE_LENGTH);
        $cookieHmac = (string)substr($cookieData, static::TOKEN_VALUE_LENGTH);

        $valid = hash_equals(
            $cookieHmac,
            hash_hmac('sha1', $cookieKey, Security::getSalt())
        );

        return $valid ? $cookieData : null;
    }
}
