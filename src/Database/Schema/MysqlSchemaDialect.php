...
class MysqlSchemaDialect extends SchemaDialect
{
    ...
    public function describeOptions(string $tableName): array
    {
        ...
        $statement = $this->driver->execute($sql, [$name]);
    ...
    public function describeForeignKeys(string $tableName): array
    {
        ...
        $statement = $this->driver->execute($sql, [$database, $name, $name]);
        $keys = [];
        foreach ($statement->fetchAll('assoc') as $row) {
...
SQL;
    ...
    public function columnDefinitionSql(array $column): string
    {
        ...
        if (isset($column['comment']) && $column['comment'] !== '') {
            ...
            $out .= ' COMMENT ' . $this->driver->quote((string)$column['comment']);
        }
    ...
    public function constraintSql(TableSchema $schema, string $name): string
    {
        ...
        if ($data['type'] === TableSchema::CONSTRAINT_UNIQUE) {
            ...
            return 'CONSTRAINT ' . $this->driver->quoteIdentifier($name) . ' CHECK (' . $data['expression'] . ')';
        }
        $out .= $this->driver->quoteIdentifier($name);
