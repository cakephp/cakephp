			...
			if ($error) {
				...
				if ($caseSensitivityTestFile->create()) {
					if (file_exists(TMP.DS.'caseSensitivityTestFile.tmp') && !file_exists(TMP.DS.'casesensitivitytestfile.tmp')) {
						trigger_error('<span style="color:orange; class="dev-heads-up">Underlying file system is case-sensitive, check query arguments are also case-sensitive.</span>', E_USER_NOTICE);
					}
