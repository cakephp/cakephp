...
class I18nExtractor {
	...
	function main() {
		...
		//die(print_r($this->files));
		$this->__extractTokens();
	}
	function help() {
		$this->__stdout('CakePHP Language String Extraction:');
		$this->hr();
		$this->__stdout('The Extract script generates .pot files with translations');
		$this->__stdout('');
		$this->hr('');
		$this->__stdout('usage: php extract.php [command] [path...]');
		$this->__stdout('');
		$this->__stdout('commands:');
		$this->__stdout('   -path [path...] Full path to directory to extract string');
		$this->__stdout('   -files [comma separated list of files]');
		$this->__stdout('   -help Shows this help message.');
		$this->__stdout('   -debug Perform self test.');
		$this->__stdout('');
	}
	function __welcome() {
		$this->__stdout('');
	...
	function __extractTokens(){
		$this->__strings = $this->__fileVersions = array();
		foreach ($this->files as $file) {
			...
			$this->singular();
			$this->singularDomain();
			$this->singularDomainCategory();
			$this->plural();
			$this->pluralDomain();
			$this->pluralDomainCategory();
			$this->category();
		}
		$this->__buildFiles();
		$this->__writeFiles();
	...
	function singular($functionname = '__') {
		$count = 0;
	...
	function singularDomain($functionname = '__d') {
		$count = 0;
		$tokenCount = count($this->__tokens);
		...
		while (($tokenCount - $count) > 7) {
			...
			if (($type == T_STRING) && ($string == $functionname) && ($firstParenthesis == "(")) {
				...
				if($this->__tokens[$position+5] === ')'){
					$comma = $this->__tokens[$position+3];
					$end = $position+5;
				} else {
					$comma = null;
					$end = $position+3;
				...
				if (($seoncdComma == "," || $seoncdComma === null) && ($endParenthesis == ")") &&
	...
	function singularDomainCategory($functionname = '__dc') {
		$count = 0;
		$tokenCount = count($this->__tokens);
		...
		while (($tokenCount - $count) > 7) {
			...
			if (($type == T_STRING) && ($string == $functionname) && ($firstParenthesis == "(")) {
				...
				if($this->__tokens[$position+7] === ')'){
					$comma = $this->__tokens[$position+5];
					$end = $position+7;
				} else {
					$comma = null;
					$end = $position+5;
	...
	function plural($functionname = '__n') {
		$count = 0;
		$tokenCount = count($this->__tokens);
	...
	function pluralDomain($functionname = '__dn') {
		$count = 0;
		$tokenCount = count($this->__tokens);
	...
	function pluralDomainCategory($functionname = '__dcn') {
		$count = 0;
		$tokenCount = count($this->__tokens);
	...
	function category($functionname = '__c') {
		$count = 0;
		$tokenCount = count($this->__tokens);
	...
	function __getInput($prompt, $options = null, $default = null) {
		...
		if($default == null) {
			$this->__stdout('');
			$this->__stdout($prompt . " $print_options \n" . '> ', false);
		} else {
			$this->__stdout('');
			$this->__stdout($prompt . " $print_options \n" . "[$default] > ", false);
		}
