 ...
 * @return string
 */
   function passwordTag($tagName, $size=20, $html_options=null)
   {
      $this->setFormTag($tagName);
      $html_options['size'] = $size;
      empty($html_options['value'])? $html_options['value'] = $this->tagValue($tagName): null;
      return sprintf(TAG_PASSWORD, $this->model, $this->field, $this->parseHtmlOptions($html_options, null, '', ' '));
   }
 ...
 * @return string
 */
   function hiddenTag($tagName, $value=null, $html_options=null)
   {
      $this->setFormTag($tagName);
      $html_options['value'] = $value? $value: $this->tagValue($tagName);
 ...
 * @return string
 */
   function fileTag($tagName, $html_options=null)
   {
      return sprintf(TAG_FILE, $tagName, $this->parseHtmlOptions($html_options, null, '', ' '));
   }
 ...
 * @return string
 */
   function areaTag($tagName, $cols=60, $rows=10, $html_options=null)
   {
      $this->setFormTag($tagName);
      $value = empty($html_options['value'])? $this->tagValue($tagName): empty($html_options['value']);
      $html_options['cols'] = $cols;
      $html_options['rows'] = $rows;
      $this->tagIsInvalid($this->model,$this->field)? $html_options['class'] = 'form_error': null;
      return sprintf(TAG_AREA, $this->model,$this->field, $this->parseHtmlOptions($html_options, null, ' '), $value);
   }
 ...
 * @return string
 */
   function checkboxTag($tagName, $title=null, $html_options=null)
   {
      $this->setFormTag($tagName);
      $this->tagValue($tagName)? $html_options['checked'] = 'checked': null;
      $title = $title? $title: ucfirst($tagName);
      return sprintf(TAG_CHECKBOX, $this->model, $this->field, $this->field, $this->field, $this->parseHtmlOptions($html_options, null, '', ' '), $title);
   }
 ...
 * @return string
 */
   function radioTags($tagName, $options, $inbetween=null, $html_options=null)
   {
      $this->setFormTag($tagName);
      $value = isset($html_options['value'])? $html_options['value']: $this->tagValue($tagName);
      $out = array();
