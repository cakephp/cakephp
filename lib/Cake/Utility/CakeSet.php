 ...
 * @link http://book.cakephp.org/2.0/en/core-utility-libraries/set.html#CakeSet::merge
 */
 ...
 * @link http://book.cakephp.org/2.0/en/core-utility-libraries/set.html#CakeSet::filter
 */
 ...
 * @link http://book.cakephp.org/2.0/en/core-utility-libraries/set.html#CakeSet::pushDiff
 */
		...
		if (!empty($array) && !empty($array2)) {
			foreach ($array2 as $key => $value) {
				if (!array_key_exists($key, $array)) {
					...
					if (is_array($value)) {
						$array[$key] = CakeSet::pushDiff($array[$key], $array2[$key]);
					}
				}
			}
		}
 ...
 * @link http://book.cakephp.org/2.0/en/core-utility-libraries/set.html#CakeSet::map
 */
		...
		if (is_array($array)) {
			...
			foreach ($array as $key => $value) {
				...
				if (is_numeric($key)) {
					...
					$out[$key] = CakeSet::_map($value, $class);
					if (is_object($out[$key])) {
						if ($primary !== true && is_array($value) && CakeSet::countDim($value, true) === 2) {
					...
					if ($primary === true) {
						...
						foreach ($value as $key2 => $value2) {
							$out->{$key2} = CakeSet::_map($value2, true);
						}
					} else {
						if (!is_numeric($key)) {
							$out->{$key} = CakeSet::_map($value, true, $key);
							...
							$out->{$key} = CakeSet::_map($value, true);
						}
					}
				} else {
					$out->{$key} = $value;
 ...
 * @link http://book.cakephp.org/2.0/en/core-utility-libraries/set.html#CakeSet::numeric
 */
 ...
 * @link http://book.cakephp.org/2.0/en/core-utility-libraries/set.html#CakeSet::enum
 */
	public static function enum($select, $list = null) {
		...
		$return = null;
		$list = CakeSet::normalize($list, false);
 ...
 * @param array $keys An array containing one or more CakeSet::extract()-style key paths
 * @return array An array of strings extracted from $keys and formatted with $format
 * @link http://book.cakephp.org/2.0/en/core-utility-libraries/set.html#CakeSet::format
 */
	public static function format($data, $format, $keys) {
		...
		for ($i = 0; $i < $count; $i++) {
			$extracted[] = CakeSet::extract($data, $keys[$i]);
		}
		$out = array();
		$data = $extracted;
		$count = count($data[0]);
 ...
 * is delegated to CakeSet::classicExtract(). Also the $path and $data arguments are
 * reversible.
 *
 ...
 * @link http://book.cakephp.org/2.0/en/core-utility-libraries/set.html#CakeSet::extract
 */
	public static function extract($path, $data = null, $options = array()) {
		...
		if (strpos($path, '/') === false) {
			return CakeSet::classicExtract($data, $path);
		}
		if (empty($data)) {
			return array();
		}
		...
		if (!isset($contexts[0])) {
			...
			if ((is_array($current) && count($data) < 1) || !is_array($current) || !CakeSet::numeric(array_keys($data))) {
				$contexts = array($data);
			}
		}
		...
		do {
			...
			foreach ($contexts as $key => $context) {
				...
				if ($token === '..') {
					...
					$context['item'] = CakeSet::extract($parent, $data);
			...
			if ($conditions) {
				foreach ($conditions as $condition) {
					...
					foreach ($matches as $i => $match) {
						if (CakeSet::matches(array($condition), $match['item'], $i + 1, $length)) {
							$filtered[$i] = $match;
						}
					}
					$matches = $filtered;
 ...
 * @return boolean
 * @link http://book.cakephp.org/2.0/en/core-utility-libraries/set.html#CakeSet::matches
 */
	public static function matches($conditions, $data = array(), $i = null, $length = null) {
		...
		if (is_string($conditions)) {
			return (bool)CakeSet::extract($conditions, $data);
		}
		foreach ($conditions as $condition) {
			if ($condition === ':last') {
				if ($i != $length) {
 ...
 * @link http://book.cakephp.org/2.0/en/core-utility-libraries/set.html#CakeSet::classicExtract
 */
				...
				foreach ($data as $j => $val) {
					if (is_int($j)) {
						...
						if (empty($tmpPath)) {
							...
							$tmp[] = CakeSet::classicExtract($val, $tmpPath);
						}
					}
				}
				return $tmp;
				...
				foreach ($data as $j => $val) {
					if (preg_match('/^' . $pattern . '/s', $j) !== 0) {
						...
						if (empty($tmpPath)) {
							...
							$tmp[$j] = CakeSet::classicExtract($val, $tmpPath);
						}
					}
				}
				return $tmp;
 ...
 * @return array
 * @link http://book.cakephp.org/2.0/en/core-utility-libraries/set.html#CakeSet::insert
 */
 ...
 * @link http://book.cakephp.org/2.0/en/core-utility-libraries/set.html#CakeSet::remove
 */
 ...
 * @link http://book.cakephp.org/2.0/en/core-utility-libraries/set.html#CakeSet::check
 */
 ...
 * @link http://book.cakephp.org/2.0/en/core-utility-libraries/set.html#CakeSet::diff
 */
 ...
 * @link http://book.cakephp.org/2.0/en/core-utility-libraries/set.html#CakeSet::contains
 */
 ...
 * @link http://book.cakephp.org/2.0/en/core-utility-libraries/set.html#CakeSet::countDim
 */
		...
		if ($all) {
			...
			if (is_array($array) && reset($array) !== false) {
				foreach ($array as $value) {
					$depth[] = CakeSet::countDim($value, true, $count + 1);
				}
			}
			$return = max($depth);
		} else {
			if (is_array(reset($array))) {
				$return = CakeSet::countDim(reset($array)) + 1;
			} else {
				$return = 1;
			}
		}
 ...
 * @return array
 * @link http://book.cakephp.org/2.0/en/core-utility-libraries/set.html#CakeSet::normalize
 */
 ...
 * to null (useful for CakeSet::merge). You can optionally group the values by what is obtained when
 ...
 * @return array Combined array
 * @link http://book.cakephp.org/2.0/en/core-utility-libraries/set.html#CakeSet::combine
 */
	public static function combine($data, $path1 = null, $path2 = null, $groupPath = null) {
		...
		if (is_array($path1)) {
			...
			$keys = CakeSet::format($data, $format, $path1);
		} else {
			$keys = CakeSet::extract($data, $path1);
		}
		if (empty($keys)) {
			return array();
		}
		...
		if (!empty($path2) && is_array($path2)) {
			...
			$vals = CakeSet::format($data, $format, $path2);
		} elseif (!empty($path2)) {
			$vals = CakeSet::extract($data, $path2);
		} else {
			$count = count($keys);
			for ($i = 0; $i < $count; $i++) {
				$vals[$i] = null;
		...
		if ($groupPath) {
			$group = CakeSet::extract($data, $groupPath);
			if (!empty($group)) {
				$c = count($keys);
				for ($i = 0; $i < $c; $i++) {
					if (!isset($group[$i])) {
 ...
 * @link http://book.cakephp.org/2.0/en/core-utility-libraries/set.html#CakeSet::reverse
 */
	public static function reverse($object) {
		...
		if ($object instanceof SimpleXMLElement) {
			...
			foreach ($keys as $key => $value) {
				if (is_array($value)) {
					$new[$key] = (array)CakeSet::reverse($value);
				} else {
					// @codingStandardsIgnoreStart Legacy junk
					if (isset($value->_name_)) {
						$new = array_merge($new, CakeSet::reverse($value));
					} else {
						$new[$key] = CakeSet::reverse($value);
					}
					// @codingStandardsIgnoreEnd
				}
			}
 ...
 * @return array
 * @link http://book.cakephp.org/2.0/en/core-utility-libraries/set.html#CakeSet::flatten
 */
 ...
 *
 * For example, unflattens an array that was collapsed with `CakeSet::flatten()`
	...
	protected static function _flatten($results, $key = null) {
		...
		foreach ($results as $k => $r) {
			...
			if (is_array($r) && !empty($r)) {
				$stack = array_merge($stack, CakeSet::_flatten($r, $id));
			} else {
 ...
 * @link http://book.cakephp.org/2.0/en/core-utility-libraries/set.html#CakeSet::sort
 */
	public static function sort($data, $path, $dir) {
		...
		$result = CakeSet::_flatten(CakeSet::extract($data, $path));
		list($keys, $values) = array(CakeSet::extract($result, '{n}.id'), CakeSet::extract($result, '{n}.value'));
 ...
 * array extracted by a CakeSet::extract() compatible path.
 *
 ...
 * @link http://book.cakephp.org/2.0/en/core-utility-libraries/set.html#CakeSet::apply
 */
	public static function apply($path, $data, $callback, $options = array()) {
		$defaults = array('type' => 'pass');
		$options = array_merge($defaults, $options);
		$extracted = CakeSet::extract($path, $data);
 ...
 */
	public static function nest($data, $options = array()) {
		...
		$return = $idMap = array();
		$ids = CakeSet::extract($data, $options['idPath']);
		...
		foreach ($data as $result) {
			...
			$id = CakeSet::get($result, $idKeys);
			$parentId = CakeSet::get($result, $parentKeys);
		...
		if ($options['root']) {
			...
			$root = CakeSet::get($return[0], $parentKeys);
		}
